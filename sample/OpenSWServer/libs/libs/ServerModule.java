...
public class ServerModule implements ServerModuleListener {
	...
	public void setOnServerModuleListener(
			ServerModuleListener mServerModuleListener) {
	...
	private class SendThread extends Thread {
		...
		@SuppressWarnings("unchecked")
		private void sendPath1(String path) throws Exception {
			File[] childFile = new File(path).listFiles();
			...
			while (true) {
				...
				jsonObjectMain.put(ServeReFlag.FLAG_CONTENT_AVAILABLE,
						available + "");
				jsonObjectMain
						.put(ServeReFlag.FLAG_CONTENTS, jsonArrayContents);

				String sendData = jsonObjectMain.toJSONString();

				mDataOutputStream.writeUTF(sendData);
				mServerModuleListener.onSendingData(childFile.length,
						available, sendData);
				if (available <= 0) {
					break;
				}
		...
		private void sendFile(String path) throws Exception {
			...
			try {
				while (fileInput.read(buffer) != -1) {
					...
					jsonObjectMain.put(ServeReFlag.FLAG_CONTENT_AVAILABLE,
							contentAvailable + "");
					jsonObjectMain.put(ServeReFlag.FLAG_CONTENT,
							Base64.encodeBytes(buffer));

					String sendData = jsonObjectMain.toJSONString();
					mDataOutputStream.writeUTF(sendData);
					mServerModuleListener.onSendingData(contentSize,
							contentAvailable, sendData);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		private void sendFileSecure(String path) throws Exception {
			...
			while (fileInput.read(buffer) != -1) {
				...
				JSONObject jsonObjectMain = new JSONObject();
				jsonObjectMain
						.put(ServeReFlag.FLAG_CONTENT_TYPE, "file_secure");
				...
				jsonObjectMain.put(ServeReFlag.FLAG_CONTENT,
						Base64.encodeBytes(mCipher.doFinal(buffer)));
				String sendData = jsonObjectMain.toJSONString();
		...
		private void sendRSAPublicKey() throws IOException {
			...
			if (publicKey == null) {
				...
				mDataOutputStream.writeUTF(Base64.encodeBytes(publicKey
						.getEncoded()));
			}
		}
		...
		private void receiveSecurekey() throws IOException {
			...
			if (!securekeyTmp.equals("null")) {
				try {
					...
					byte[] secureKeyBuffer = mCipher.doFinal(Base64
							.decode(securekeyTmp));
					secureKey = new SecretKeySpec(secureKeyBuffer, 0,
							secureKeyBuffer.length, "AES");
